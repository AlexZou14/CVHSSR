...
class BlindImageRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(BlindImageRestorationModel, self).__init__(opt)

        # define network
        self.net_g = define_network(deepcopy(opt['network_g']))
        self.net_g = self.model_to_device(self.net_g)
        self.print_network(self.net_g)
        self.net_k = define_network(deepcopy(opt['network_k']))
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        k_pred = self.net_k(self.lq[:,:3,:,:])
        inputs = [k_pred,self.lq]
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                k_pred = self.net_k(self.lq[i:j][:,:3,:,:])
                inputs = [k_pred, self.lq[i:j]]
                pred = self.net_g(inputs)
    ...
    def load_K(self):
        load_path_K = self.opt['path_k']['pretrain_model_K']
        if load_path_K is not None:
            print('Loading model for K [{:s}] ...'.format(load_path_K))
            model_dict = self.net_g.state_dict()
            pretrained_dict = torch.load(load_path_K)
#            for k, v in pretrained_dict.items():
#                print(k)
#                if 'module.'+k in model_dict:
#                   print(k)
#                   pretrained_dict = {'module.'+k:v}
        for k, v in deepcopy(pretrained_dict).items():
            if k.startswith('kernel_estimation.'):
                pretrained_dict['module.'+k[18:]] = v
                pretrained_dict.pop(k)
                            
        self.net_k.load_state_dict(pretrained_dict, strict=True)
