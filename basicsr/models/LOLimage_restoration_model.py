...
class LOLImageRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(LOLImageRestorationModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('map_opt'):
            map_type = train_opt['map_opt'].pop('type')
            cri_map_cls = getattr(loss_module, map_type)
            self.cri_map = cri_map_cls(**train_opt['map_opt']).to(
                self.device)
        else:
            self.cri_map = None
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.cri_map:
            l_map = 0.
            for m in maps:
                l_map += self.cri_map(m)
                
            # print('l pix ... ', l_pix)
            l_total += l_map
            loss_dict['l_map'] = l_map
